...
class AvgMemUsage extends TableWidget
{
    ...
    public function table(Table $table): Table
    {
        ...
        $data = $query->groupBy('api_test_results.query_id', 'queries.name')
            ...
            ->where('api_test_results.mem_usage', '>', 0)
            ->selectRaw('
                ...
                (
                    (
                        avg(case when api_test_results.api_type = \'rest\' then api_test_results.mem_usage end)
                        - avg(case when api_test_results.api_type = \'integrated\' then api_test_results.mem_usage end)
                    ) / nullif(avg(case when api_test_results.api_type = \'rest\' then api_test_results.mem_usage end), 0)
                ) * 100 as vs_rest,
                (
                    (
                        avg(case when api_test_results.api_type = \'graphql\' then api_test_results.mem_usage end)
                        - avg(case when api_test_results.api_type = \'integrated\' then api_test_results.mem_usage end)
                    ) / nullif(avg(case when api_test_results.api_type = \'graphql\' then api_test_results.mem_usage end), 0)
                ) * 100 as vs_graphql
            ')
            ...
            ->columns([
                ...
                TextColumn::make('avg_integrated_mem_usage')
                    ...
                    ->color(fn($record) => PerformanceBadge::getColor(
                        (float)$record['avg_integrated_mem_usage'] ?? 0,
                        (float)$record['avg_rest_mem_usage'] ?? 0,
                        (float)$record['avg_graphql_mem_usage'] ?? 0
                    ))
